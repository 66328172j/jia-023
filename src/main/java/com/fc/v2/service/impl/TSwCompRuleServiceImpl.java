...
import com.fc.v2.service.ITSwCompRuleService;

/**
 * 补偿费减免比例判定规则 Service业务层处理（rule-eval 形状）
 *
 * @author fuce
 * @date 2026-09-14
 */
@Service
public class TSwCompRuleServiceImpl implements ITSwCompRuleService {

    /** 优先级降序；并列按 ruleCode 降序（确定性） */
    private static final Comparator<TSwCompRule> PRIORITY_THEN_CODE_DESC = new Comparator<TSwCompRule>() {
        @Override
        public int compare(TSwCompRule a, TSwCompRule b) {
    ...
    private TSwCompRuleMapper swCompRuleMapper;
    ...
    @Override
    public TSwCompRule selectTSwCompRuleById(Long id) {
        return this.swCompRuleMapper.selectById(id);
    }

    @Override
    public List<TSwCompRule> listAvailable(Date at) {
        if (at == null) {
            return new java.util.ArrayList<TSwCompRule>();
        }
        return this.swCompRuleMapper.selectList(new QueryWrapper<TSwCompRule>().eq("del_flag", 0));
    }
    ...
    public int evaluate(String ruleCode, BigDecimal input, Date at) {
        ...
        TSwCompRule rule = findRule(ruleCode, at);
    ...
    public int evaluateTop(BigDecimal input, Date at) {
        ...
        List<TSwCompRule> avail = listAvailable(at);
        if (avail.isEmpty()) {
            return 0;
        }
    ...
    public boolean usable(Long id, Date at) {
        ...
        TSwCompRule r = this.swCompRuleMapper.selectById(id);
        return r != null;
    }
    ...
    public int countAvailable(Date at) {
        ...
        return this.swCompRuleMapper
                .selectList(new QueryWrapper<TSwCompRule>().eq("del_flag", 0)).size();
    }

    private TSwCompRule findRule(String ruleCode, Date at) {
        List<TSwCompRule> hit = this.swCompRuleMapper.selectList(new QueryWrapper<TSwCompRule>()
                .eq("del_flag", 0).eq("rule_code", ruleCode));
        TSwCompRule newest = null;
        for (TSwCompRule r : hit) {
    ...
    private BigDecimal resolveLimit(TSwCompRule r, int tier) {
        if (tier == 1) {
            return r.getTh1Max();
        }
        if (tier == 2) {
    ...
    /** 档案级默认阈值（明细未维护时的回退来源） */
    private BigDecimal defaultThreshold(TSwCompRule r, int tier) {
        return r.getTh1Max();
    }

    /** 分档：等于上限取高一档 */
    private int levelOf(TSwCompRule rule, BigDecimal input) {
